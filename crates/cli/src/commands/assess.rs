//! The `assess` command: PDF in, graduation report out.

use std::fs;
use std::io::Write as _;

use audit_app::import_raw_courses;
...
pub fn run(args: AssessArgs, palette: &Palette, use_color: bool) -> Result<(), CliError> {
    ...
    let sp = spinner::start("成績表を解析中…");
    let prepared = match prepare(&bytes, &args) {
        Ok(prepared) => {
            sp.done(
...
fn prepare(bytes: &[u8], args: &AssessArgs) -> Result<Prepared, CliError> {
